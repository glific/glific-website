...
<header>
	...
	if ( ! empty( $banner_content ) ) :
		if ( ! empty( $banner_content['details'] ) && ! empty( $banner_content['link'] ) ) :
			...
			<nav class="bg-theme-pewter d-flex flex-row justify-content-center navbar fixed-top banner-header py-3.5">
				...
				<a class="fz-14 leading-21 text-theme-primary font-heebo-bold text-decoration-none ml-10" href="<?php echo $banner_content['link']; ?>">Sign up
				</a>
			</nav>
			<?php
		endif;
	...
	<nav class="navbar navbar-expand-xl py-4.5 px-6 py-xl-6.6 px-xl-8 fixed-top bg-white box-shadow-dark-10 <?php echo ! empty( $banner_content['details'] ) && ! empty( $banner_content['link'] ) ? 'mt-10' : ''; ?>">
		...
		<div class="ml-auto d-xl-none d-flex align-items-center">
			...
			if ( ! empty( $demoURL ) ) :
				?>
				<a class="bg-theme-secondary font-heebo-bold px-6 py-4 fz-18 leading-27 text-white text-decoration-none glific-button-border d-xl-none mr-9 mr-md-11 d-flex align-items-center message-us" target="_blank" href="<?php echo $demoURL; ?>">
		...
		<div class="ml-xl-auto d-flex flex-row">
			...
			if ( ! empty( $demoURL ) ) :
				?>
				<a class="bg-theme-secondary font-heebo-bold px-8 py-4 fz-18 leading-27 text-white text-decoration-none ml-11 glific-button-border d-none d-xl-flex align-items-center message-us" target="_blank" href="<?php echo $demoURL; ?>">
				<?php echo file_get_contents(get_template_directory(). '/dist/images/whatsapp.svg') ?> 
				<span class="ml-4">Message us</span>
				</a>
				<?php
